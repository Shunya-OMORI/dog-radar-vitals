"""RCG(50ch, MMECG) -> R波heatmap（疎なイベント検出）の学習ループ。`mmecg_trainer.py`と対になる。"""
...
def train_mmecg_rpeak(config: dict, run_dir: Path, repo_root: Path) -> None:
...
def evaluate_mmecg_rpeak(run_dir: Path, config: dict, repo_root: Path) -> dict[str, float]:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    data_cfg = config["data"]
    raw_root = repo_root / data_cfg["raw_root"]
