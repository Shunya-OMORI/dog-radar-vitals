...
| Pan-Tompkins型 searchback | 0.303（悪化） | 13.67 ms |

**重要な発見**: 応急対応のpaired_dedupは，NeuroKit2で正解ラベルを付け直した新モデルには
...
適用しない）．リズム一貫性ベース（DP）はF1で旧手法をわずかに上回るが，RR-MAEでは
劣る．Pan-Tompkins型は文献上の裏付けはあるが，ECGの生波形ではなくheatmap出力という
異なる入力特性への移植では今回はうまく機能せず，パラメータの再調整が今後の課題である．
