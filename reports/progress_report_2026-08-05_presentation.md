...
2. **空間情報を使うと，検出性能と引き換えにタイミング精度が上がる．** GNN は F1 で劣るが RR間隔では 1.6 倍良い．
...
**恒久対応**（進行中）: 外部で検証済みのR波検出アルゴリズム NeuroKit2（Makowski et
al., 2021）を正解ラベルの再生成に採用した．全91トライアルに対してNeuroKit2で
異常に短いRR間隔（生理的にありえない値）が0件だったのに対し，旧来の検出器は
曖昧な結果だったことを確認済み．空間GNNは正解を都度計算する実装のため先に
8epochプローブを実施し，**新旧チェックポイントを同一のNeuroKit2正解に対して
公平に評価**したところ，旧チェックポイント（旧ラベルで学習）はF1 0.510 /
RR-MAE 10.27 ms，NeuroKit2ラベルで学習し直した新チェックポイントはF1 0.549 /
RR-MAE 10.91 ms / RMSSD-MAE 18.06 ms（旧20.90 msから改善）となり，本番エポック数
（50epoch）での再学習に進める根拠を得た．現在，空間GNNの本番再学習と，生信号
Anchor CNN側の全データNeuroKit2再ラベル付け（キャッシュを使う実装のため時間を
要する）をGPUキューで実行中．完了後，本節の表と関連図を更新する．
...
2. **検出性能とタイミング精度の両立**．現状は CNN（F1 0.570）と GNN（RR-MAE 9.67 ms）で得意分野が分かれている．統合する小さなモジュール（NCP では学習が破綻したが，3 パラメータの線形結合は機能した）を設計する．
