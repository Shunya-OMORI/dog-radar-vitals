...
## III-6c. 前処理の横断比較と現行の生産用構成確定（2026-08-06/07）
...
までは保証しない**という教訓として記録する。現行の生産用構成は
**config274（前処理なし）の空間GNN + Pan-Tompkins型 searchback（再校正後）** に確定した
（F1=0.687, RR-MAE=10.69ms, RMSSD-MAE=16.27ms）。
...
sigma幅チューニングは打ち切り，**現行の生産用構成はconfig274(sigma=10)+
adaptive_searchbackのまま変更なし**。
