...
| 2026-08-21 | `radarODE-MTL/queue/logs/010_*`, `011_*` | `queue/pending/010_radarode_leakage_armA_leaky.sh`, `011_..._armB_clean.sh` | radarODE-MTL 公開実装の分割がファイル単位で、論文の記述(被験者単位11分割)と一致しないことをコード読解で確認。被験者リークが再現ギャップの主因かを検証。テスト集合(file 80-85=subject 30)と学習ファイル数(24)を固定し、学習側6ファイルの出所だけをテスト被験者(A)/別被験者(B)で入れ替える単一変数比較。12 epoch・bs16に縮小 | **仮説は支持されず**。Arm A: test Corr 0.9374・PPI誤差 18.3ms、Arm B: Corr 0.9238・PPI誤差 19.7ms。差は Corr +0.014・PPI −1.4ms で、論文値(PPI 8.5ms)との隔たりを説明できない。副産物として、両armとも Corr 0.92〜0.94 と論文の PCC 87.4% を上回っており、**この指標が平均波形で飽和している**ことを裏づけた(入力を見ないモデルの拍単位相関 0.883 と整合) | 再現ギャップの残る候補は学習量(12 vs 200 epoch)・データ量(24 vs 77ファイル)・SST前処理(ssqueezepy vs MATLAB wsst)・セグメント仕様 |
| 2026-08-22 | `reports/b1_classical_baseline.log` | `radarODE-MTL/scripts/evaluate_classical_baseline_hrv.py`(新規) | 学習を使わない古典ベースライン(B1)。空間GNNの数値が「レーダから学習した成果」だと言うために必要(R3の自明ベースライン併記)。先頭で既知値の再現確認も実施 | **再現確認は合格**(F1 0.750 / RR 8.43ms / RMSSD 10.35ms、報告値 0.749 / 8.41 / 10.27)。periodic(入力を見ない): F1 0.128・RR 37.22ms。classical(最良チャネル+Pan-Tompkins型包絡、最良しきい値): F1 0.121・RR 17.89ms。**古典手法のF1は入力を見ない自明ベースラインと同水準**で、空間GNN(0.750)が決定的に上回る | RR間隔MAEは対応づいた拍だけで計算するため、検出できていない手法ほど有利に出る。資料では必ずF1と併記する |
| 2026-08-22 | `runs/*_lodo284_fold{0..10}_*`, `reports/spatial_gnn_lodo_cv_eval.log` | `scripts/run_spatial_gnn_lodo_cv.py`(新規)、config284から被験者分割のみ変更 | 先行研究(radarODE系)が論文で述べる被験者単位11分割に合わせる(B3)。相手の実装を再現するのではなく相手のプロトコルをこちらが採用する方針。チェックポイントは全foldで epoch_004.pt に固定 | **F1 0.567±0.246**(単一split 0.750)、**RR間隔MAE 10.50±2.07ms**(同 8.43ms)、RMSSD MAE 16.94±14.63ms(同 10.35ms)。fold間のF1は 0.113(subj9)〜0.906(subj17)と大きく振れる。単一splitと同じ3名(17/29/30)のfoldだけ平均すると F1 0.761・RR 9.75ms で単一splitとほぼ一致し、**分割手続きではなく「たまたま易しい3名がテストに入っていた」ことが原因**と特定できた | **RR間隔MAEは2msしか悪化せず頑健、F1は大きく落ちる。**主指標をRR間隔に置いた判断が結果的に正しかった。以後の報告値はLOSOを正式値にする |

| 2026-08-23 | `reports/b6_prior_work_metrics_loso.log` | `evaluate_spatial_gnn_hrv.py --prior_work_metrics`(追加) | B6: 先行研究(radarODE)の指標定義に揃えて11 fold LOSOを測り直す。MDR(規則1+2、許容0.15s)・R波タイミング誤差・許容0.15sのF1。**原著MDRの規則(3)「振幅が正解より30%低い」は我々がheatmapを出すため未実装＝我々に有利な値** | **検出とタイミングで明確に負けた**。MDR 33.40±26.98%(radarODE 0.18%, radarODE-MTL 4.31%)、R波タイミング|誤差| 36.69±16.80ms(同 6ms, 7.2ms)。一方 RR間隔MAE 10.50±2.07ms は radarODE-MTL の PPI誤差 8.5ms と同水準。**符号つき平均(bias)が被験者ごとに −60.96〜+59.72 ms と大きく振れ、全体平均は +0.74ms**。subject 1/2/5 では \|bias\|/平均\|誤差\| が 0.91〜1.00 で、タイミング誤差のほぼ全部が一定ずれ | 2026-08-07 のプール解析(mean 6.54ms)で系統ラグが見えなかったのは、**被験者ごとのずれが符号違いで相殺していた**ため。定数シフトが効かなかった理由も説明できる。次は被験者ごとのオフセット較正の可否を検証する |
| 2026-08-23 | `runs/20260823-13*/14*_ecg_rpeak_spatial_gnn`(3件), `reports/b4_seed_variance_eval.log` | `configs/experiments/2843`〜`2845`(284からseedのみ変更) | B4: 報告値のseed間ばらつきを数値化。単一split・epoch_004固定・評価経路も同一 | F1 0.707±0.042(範囲 0.652〜0.750)、RR間隔MAE 9.07±0.67ms(範囲 8.43〜9.99)、MDR 19.39±2.58%。**既報告の seed42 (F1 0.750 / RR 8.43ms) は4本中の最良値だった** | 報告は平均±標準偏差に改める。ばらつきの大きさは 被験者選び(F1 ±0.246) >> seed(±0.042) であり、**分割の方が支配的**と定量化できた |
| 2026-08-25 | `reports/chen_metrics_loso.log` | `evaluate_spatial_gnn_hrv.py`にChen ら(IEEE T-MC 2024)の指標(正規化タイミング誤差の中央値・90パーセンタイル)を追加 | 目的をHRV(RR間隔とその変動)に据え直し、検出の許容幅を先行研究と揃える。Chen ら本文4.3.1はAAMI推奨に従い許容150ms、radarODEのMDR規則(1)も0.15s。**我々だけが50msで測っていた** | **同じチェックポイント・同じ後処理のまま許容幅だけ150msにすると F1 0.567→0.729(11-fold LOSO)。** モデルは不変。物差しの定義だけで0.16動く。被験者別ではRR間隔MAEが全員8.25〜14.29msと安定しており、崩れているのは検出(recall)だけと判明 | 後段処理の系統比較(043)へ |
| 2026-08-25 | `reports/consensus_detector.log` | `evaluate_consensus_detector.py`(新規)、`mmecg_ppi.py: detect_consensus_beat_times`を初めてR波検出器として評価 | radarODEのAlgorithm 1(50ch各々にピーク検出→KDE多数決)が非学習で拍を切り出している点に着目し、それ自体の検出性能を測る | **F1 0.091 / RR間隔MAE 32.81 ms。** 1チャネル+Pan-Tompkinsの古典(F1 0.121)より弱い。「非学習の多数決が学習より強い」という仮説は棄却。学習ベース検出(F1 0.567)が圧倒的 | 入力スペクトル診断で理由が判明(後述) |
| 2026-08-25 | `reports/sweep_f1_150ms.log`, `reports/sweep_rmssd_150ms.log` | `evaluate_height_sweep.py`にHRV指標(平均IBI/RMSSD/SDNN/pNN50)・`--tol_ms`・`--select_metric`・`--rr_correction`を追加 | 検出性能とHRV精度が同じ動作点で両立するかを、しきい値の全域曲線で測る | **両立しない。** F1最大はh=0.10(0.831)、RMSSD誤差最小はh=0.40(13.00ms、一定RRのみ41.83ms)、平均IBI誤差最小はh=0.30(11.27ms)。valでしきい値を選ぶ正式値はF1基準0.841±0.129、RMSSD基準でRMSSD誤差16.85ms。**被験者ごとの最適しきい値が0.05〜0.60と散らばり、val 1名では転移しない** | 資料の中核の図に採用 |
| 2026-08-25 | `reports/viterbi_kubios_lodo.log`, `reports/viterbi_candheight.log`, `reports/adaptive_searchback_loso.log` | `rpeaks.py`に`extract_peaks_viterbi`・`correct_rr_artifacts_kubios`(新規) | 後段の検出・復号を古典的手法に差し替えて11-fold LOSOで横並び比較。学習済みモデル・評価手順は共通、変えたのは後段だけ(R4) | **5手法すべて改善せず、線形しきい値0.3が最良。** Viterbi(Viterbi 1967/Coast ら1990): F1 0.729→0.533、RMSSD 16.94→24.45ms。Kubios(Lipponen & Tarvainen 2019): h≥0.15で微改善(h=0.40で13.00→12.51ms)だが低しきい値では害。Pan-Tompkins適応しきい値: 改善せず。**ただし被験者9(MDR 92.6%)単独ではViterbiがF1 0.132→0.798と劇的に効く**——検出が壊滅した場合のみ有効 | 後段は試し切り。前処理の診断へ |
| 2026-08-25 | `reports/input_spectrum.log` | `scripts/diagnose_input_spectrum.py`(新規) | R5に従い、前処理の対策を作る前に入力を測る。Wang ら(Sensors 2025)は疎分離で呼吸と心拍を分けており、同じことをする価値があるかを判断する | **呼吸成分は既に落ちている**(呼吸/心拍パワー比0.05)ので疎分離は不要。**より重要な発見: RCGのエネルギーは98.09%が[3,25)Hzの高域にあり、スペクトルピークから読める心拍数は正解ECGと平均53.3 bpmずれる。** 周波数解析ベースの手法がこの入力で働かない理由が定量化された(50ch多数決F1 0.091と整合)。過去の[1,25]Hzバンドパス悪化(248:0.258→254:0.217)も、高域がノイズでなくR波の急峻さを担う情報だったためと説明がつく | 前処理も伸びしろなしと判断 |
| 2026-08-25 | `runs/*_lodo295_posw10_*`, `reports/lodo295_posw10_eval.log`, `reports/sweep_posw10.log` | `losses.py: heatmap_weighted_bce_loss`(新規)、config295(284からtrain.lossとpos_weightのみ変更) | precision 0.93〜1.00に対しrecallが落ちる=「拍でない」側に偏っている。σ=15msの教師では正例の実効重みが全体の約4.7%。その逆数(約21)の中庸をとりpos_weight=10 | **検出が大幅改善: F1(150ms) 0.729→0.847、MDR 33.40%→12.13%、F1の被験者間標準偏差0.251→0.088。** ただし**RMSSD誤差は改善せず**(16.94→17.99ms、しきい値を選び直しても13.87ms止まりで284のh=0.40固定12.51msに届かない、一定RRのみ41.83ms)。**改善したのは検出側だけ。** なお単一split・10epochのプローブ(035)では逆にRMSSDが悪化して見え、LOSO本学習で結論が逆転した——**R2のプローブは方向性の判断に足りないことがある** | pos_weight=5で切り分け(047) |
| 2026-08-25 | `reports/downsample_probe.log` | config296/297(284から`model.n_downsample`のみ3→2/1) | 入力の98%が高域にあるという診断から、backboneの8倍ダウンサンプリング(200Hz→実質25Hz、40ms/フレーム)がR波位置の決定に粗すぎると推論。config269(mmECG側)で段数削減が奏功した先例あり | **棄却。** 2段: F1 0.750→0.573、RR 8.43→10.99ms、RMSSD 10.35→25.92ms。1段: F1 0.010で完全崩壊(44試行中1試行しかHRV計算不可)。**8倍ダウンサンプリングは情報を捨てているのではなく受容野を確保して学習を安定させる役割を果たしていた。** config269で効いたのは、あちらが1心拍タイトクロップという別の入力形式だったため | モデル構造も試し切り |
